...
#' @export
#'
births.mard.m2 <- function(dat, at){
  ...
  ## Update Attr
  if (nBirths > 0) {
    dat <- setBirthAttr.mard.m2(dat, at, nBirths.B, nBirths.W)
  }
  ...
  if (nBirths > 0) {
    ...
    new.racepref <- dat$attr$racepref[newIds]
...
setBirthAttr.mard.m2 <- function(dat, at, nBirths.B, nBirths.W) {
  ...
  #Partner Race Preference 
  
  dat$attr$racepref[newIds[newB]] <- sample(c("BB", "BBW", "BW"),
                                              nBirths.B, replace = TRUE,
                                              prob = dat$param$racepref.B.prob)
  dat$attr$racepref[newIds[newW]] <- sample(c("WB", "WBW", "WW"),
                                              nBirths.W, replace = TRUE,
                                              prob = dat$param$racepref.W.prob)
