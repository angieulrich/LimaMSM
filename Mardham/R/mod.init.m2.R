...
#' @keywords module
#'
initialize.mard.m2 <- function(x, param, init, control, s) {
  ...
  circ[ids.W] <- sample(apportion.lr(num.W, 0:1, 1 - param$circ.W.prob))
  dat$attr$circ <- circ
  ...
  # Proportions by Partner Race Preference
  racepref <- dat$nw[[1]] %v% "race.pref"
  dat$attr$racepref <- racepref
...
#' @inheritParams initialize.mard.m2
#'
#' @return
