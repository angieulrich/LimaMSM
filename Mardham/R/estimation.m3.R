...
#' @export
#'
calc_nwstats.mard.m3 <- function(time.unit = 7,
                              num.B,
                              num.W,
                              deg.mp.B,
                              deg.mp.W,
  ...
  ##NOT USING THIS NODEMIX SEQUENCE ANYMORE
  ...
  # INSTEAD, WE USE THIS TO HELP CALC SQRTADIFF:
  # # Number of mixed-race partnerships, with balancing to decide
  edges.m.B2W <- totdeg.m.by.race[1] * (1 - racepref.B.prob[1])
  edges.m.W2B <- totdeg.m.by.race[2] * (1 - racepref.W.prob[3])
  edges.het.m <- switch(balance,
                        black = edges.m.B2W,
                        white = edges.m.W2B,
                        mean = (edges.m.B2W + edges.m.W2B) / 2)
  ...
  # expected number of BB, BW, WW partnerships 
   edges.mix.m <- c(edges.hom.m[1], edges.het.m, edges.hom.m[2])
  
  # Sqrt absdiff term for age #NOW USING edges.mix.m instead of edges.nodemix.m
  ...
  if (age.method == "heterogeneous") {
    sqrt.adiff.m <- edges.mix.m * c(sqrt.adiff.BB[1],
                                        sqrt.adiff.BW[1],
                                        sqrt.adiff.WW[1])
  }
  if (age.method == "homogeneous") {
    weighted.avg <- sum(edges.mix.m * c(sqrt.adiff.BB[1],
                                            sqrt.adiff.BW[1],
                                            sqrt.adiff.WW[1])) / sum(edges.mix.m)
    sqrt.adiff.m <- edges.mix.m * weighted.avg
  }
  ...
  if (dur.method == "homogeneous") {
    weights <- edges.mix.m / sum(edges.mix.m)
  ...
  ##NOT USING THIS NODEMIX SEQUENCE ANYMORE
  # Number of mixed-race partnerships, with balancing to decide
  edges.p.B2W <- totdeg.p.by.race[1] * (1 - racepref.B.prob[1])
  edges.p.W2B <- totdeg.p.by.race[2] * (1 - racepref.W.prob[3])
  edges.het.p <- switch(balance,
                        black = edges.p.B2W, 
                        white = edges.p.W2B,
                        mean = (edges.p.B2W + edges.p.W2B) / 2)
  ...
  # expected number of BB, BW, WW partnerships
  edges.mix.p <- c(edges.hom.p[1], edges.het.p, edges.hom.p[2])
  
  # Sqrt absdiff term for age
  if (age.method == "heterogeneous") {
    sqrt.adiff.p <- edges.mix.p * c(sqrt.adiff.BB[2],
                                        sqrt.adiff.BW[2],
                                        sqrt.adiff.WW[2])
  }
  if (age.method == "homogeneous") {
    weighted.avg <- sum(edges.mix.p * c(sqrt.adiff.BB[2],
                                            sqrt.adiff.BW[2],
                                            sqrt.adiff.WW[2])) / sum(edges.mix.p)
    sqrt.adiff.p <- edges.mix.p * weighted.avg
  }
 ...
 # stats.p <- c(edges.p, totdeg.p.by.dm[c(2, 4)], conc.p.by.race, sqrt.adiff.p)
  ...
  if (dur.method == "homogeneous") {
    weights <- edges.mix.p / sum(edges.mix.p)
  ...
  # Number of mixed-race partnerships, with balancing to decide
  edges.i.B2W <- totdeg.i.by.race[1] * (1 - racepref.B.prob[1])
  edges.i.W2B <- totdeg.i.by.race[2] * (1 - racepref.W.prob[3])
  edges.het.i <- switch(balance,
                        black = edges.i.B2W, 
                        white = edges.i.W2B,
                        mean = (edges.i.B2W + edges.i.W2B) / 2)
  ...
  # expected number of BB, BW, WW partnerships
  edges.mix.i <- c((totdeg.i.by.race[1] - edges.het.i) / 2,
                       edges.het.i,
                       (totdeg.i.by.race[1] - edges.het.i) / 2)
  
  if (age.method == "heterogeneous") {
    sqrt.adiff.i <- edges.mix.i * c(sqrt.adiff.BB[3],
                                        sqrt.adiff.BW[3],
                                        sqrt.adiff.WW[3])
  }
  if (age.method == "homogeneous") {
    weighted.avg <- sum(edges.mix.i * c(sqrt.adiff.BB[3],
                                            sqrt.adiff.BW[3],
                                            sqrt.adiff.WW[3])) / sum(edges.mix.i)
